...
const ttStyle = {
  contentStyle: {
    ...
    borderRadius: 8, ...mono, fontSize: 11,
...
function injectStyles() {
  ...
  s.textContent = `
    ...
    .aq-card:hover { border-color: rgba(0,245,212,0.18) !important; transform: translateY(-1px); }
    .aq-card { transition: border-color .2s, transform .2s, box-shadow .2s; }
    .aq-card:hover { box-shadow: 0 8px 32px rgba(0,0,0,0.4) !important; }
    .aq-badge:hover { opacity: .85; }
  ...
  return (
    <span style={{ display:'inline-flex', alignItems:'center', gap:5 }}>
      <span style={{
        ...
        animation: 'aq-blink 2s ease-in-out infinite',
      }}/>
...
function Label({ children, style = {} }) {
  return (
    <div style={{
      ...mono, fontSize: 9, letterSpacing: '.2em',
...
export default function Portfolio() {
  ...
  useEffect(() => {
    fetchPortfolioData();
    const iv = setInterval(fetchPortfolioData, 10000);
  ...
  if (!loading && error) return (
    <div style={{
      background: T.surface, border: `1px solid rgba(244,63,94,0.2)`,
      borderRadius: 16, padding: 56, textAlign: 'center',
      animation: 'aq-fadein .4s ease',
    }}>
      ...
      <div style={{ ...mono, fontSize: 11, color: T.slate, marginBottom: 24 }}>
        Check backend connection or session
      </div>
      <button className="aq-retry" onClick={fetchPortfolioData} style={{
        padding: '8px 28px', background: 'rgba(0,245,212,0.08)',
        color: T.cyan, border: `1px solid ${T.cyan}40`, borderRadius: 8,
        ...mono, fontSize: 12, cursor: 'pointer', transition: 'background .2s',
      }}>Retry</button>
    </div>
  );
    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
      <div style={{ background: T.surface, border: `1px solid ${T.border}`, borderRadius: 16, padding: 28 }}>
        ...
        <div style={{ display: 'flex', gap: 28 }}>
          {[...Array(3)].map((_, i) => <div key={i} style={{ flex: 1 }}><Sk h={8} w="60%" /><div style={{height:6}}/><Sk h={20}/></div>)}
        </div>
      </div>
    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: 14, animation: 'aq-fadein .35s ease' }}>
        ...
        <div style={{ position:'relative', display:'grid', gridTemplateColumns:'1fr auto', gap:24, alignItems:'start' }}>
          ...
          <div>
            ...
            <div style={{ display:'flex', alignItems:'baseline', gap:14, flexWrap:'wrap' }}>
              <Counter
                value={typeof hero.totalValue === 'number' ? hero.totalValue : 0}
                decimals={2}
                style={{ ...mono, fontSize: 46, fontWeight: 800, color: T.cyan, lineHeight: 1, letterSpacing: '-1px' }}
              />
              <span style={{
                ...mono, fontSize: 12, fontWeight: 700, padding: '4px 12px',
                borderRadius: 20,
            ...
            <div style={{ display:'flex', gap:36, flexWrap:'wrap' }}>
              {[
                ...
                <div key={label}>
                  ...
                  <div style={{ ...mono, fontSize: 17, fontWeight: 700, color: up ? T.green : T.red }}>
                    {value}
                  </div>
                </div>
              ))}
      ...
      <div style={{ display:'grid', gridTemplateColumns:'1fr 1.5fr', gap:14 }}>
        ...
        <div className="aq-card" style={{
          ...
          {allocations.length === 0 ? (
            <div style={{ ...mono, fontSize:12, color:T.slate, textAlign:'center', padding:'50px 0', opacity:.6 }}>
              No positions yet
            </div>
          ) : (
            <>
              <ResponsiveContainer width="100%" height={175}>
                <PieChart>
                  <Pie
                    data={allocations} dataKey="pct" nameKey="name"
                    innerRadius={54} outerRadius={74}
                    paddingAngle={2} labelLine={false}
                    label={<PieLabel />}
                  >
                    {allocations.map((e, i) => (
                      <Cell key={i} fill={e.color || PALETTE[i % PALETTE.length]}
        ...
        <div className="aq-card" style={{
          ...
          <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center', marginBottom:16 }}>
            ...
            {equityCurve.length >= 2 && (
              <span style={{ ...mono, fontSize:9, color:T.slate }}>30d</span>
            )}
          </div>
          {equityCurve.length < 2 ? (
            <div style={{ ...mono, fontSize:12, color:T.slate, textAlign:'center', padding:'50px 0', opacity:.6 }}>
            ...
            <ResponsiveContainer width="100%" height={195}>
              <AreaChart data={equityCurve} margin={{ top:4, right:4, left:0, bottom:0 }}>
                ...
                <XAxis dataKey="t" tick={{ ...mono, fontSize:9, fill:T.slate }}
                  axisLine={false} tickLine={false} interval="preserveStartEnd" />
                <YAxis tick={{ ...mono, fontSize:9, fill:T.slate }}
                  axisLine={false} tickLine={false}
                  tickFormatter={v => `$${(v/1000).toFixed(0)}k`} width={42} />
                <Tooltip {...ttStyle} formatter={v => [`$${parseFloat(v).toLocaleString()}`, 'Value']} />
                <Area type="monotone" dataKey="v" stroke={T.purple} strokeWidth={1.5}
                  fill="url(#aqEqGrad)" dot={false}
                  activeDot={{ r:4, fill:T.purple, stroke:'var(--surface)', strokeWidth:2 }} />
              </AreaChart>
            </ResponsiveContainer>
          )}
        </div>
      ...
      {risks.length > 0 && (
        <div style={{ display:'grid', gridTemplateColumns:`repeat(${risks.length},1fr)`, gap:12 }}>
          {risks.map((r, i) => (
            <div key={i} className="aq-card" style={{
              background: r.warn ? 'rgba(244,63,94,0.04)' : 'rgba(0,245,212,0.03)',
              border: `1px solid ${r.warn ? 'rgba(244,63,94,0.2)' : 'rgba(0,245,212,0.1)'}`,
              borderRadius:12, padding:'16px 20px',
            }}>
              <Label style={{ marginBottom:8 }}>{r.label}</Label>
              <div style={{ ...mono, fontSize:22, fontWeight:800, color:r.warn ? T.red : T.cyan, letterSpacing:'-0.5px' }}>
                {r.value}
              </div>
              ...
              <div style={{
                ...
                background: r.warn ? `linear-gradient(90deg,${T.red}40,transparent)` : `linear-gradient(90deg,${T.cyan}30,transparent)`,
              }}/>
            </div>
          ))}
        </div>
            ...
            <div style={{ display:'flex', gap:6, marginLeft:'auto', alignItems:'center' }}>
              {['all','long','short'].map(f => (
                <button key={f} className="aq-filter" onClick={() => setSideFilter(f)} style={{
                  ...
                  border:'1px solid',
                  borderColor: sideFilter === f ? T.cyan : 'rgba(255,255,255,0.08)',
              ...
              <span style={{ ...mono, fontSize:9, color:T.slate, marginLeft:6 }}>
                {filtered.length} pos
              </span>
            </div>
          )}
        ...
        {activeTab === 'positions' && (
          filtered.length === 0 ? (
            <div style={{ textAlign:'center', padding:'48px 0', color:T.slate, ...mono, fontSize:12, opacity:.6 }}>
              No {sideFilter !== 'all' ? sideFilter : ''} positions
            </div>
          ) : (
                ...
                <thead>
                  <tr>
                    {[
                      ...
                      <th key={label} style={{
                        ...mono, fontSize:9, letterSpacing:'.18em', textTransform:'uppercase',
                        color:T.slate, textAlign:'left', paddingBottom:12, width:w,
                        fontWeight:400,
                      }}>{label}</th>
                    ))}
                  </tr>
                ...
                <tbody>
                  {filtered.map((p, i) => (
                    <tr key={i} className="aq-row">
                      {/* Asset */}
                      <td style={{ ...mono, fontSize:13, fontWeight:700, color:'var(--text)', padding:'12px 0', verticalAlign:'middle' }}>
                        <div style={{ display:'flex', alignItems:'center', gap:8 }}>
                          <div style={{
                            ...
                            fontSize:9, fontWeight:700, color:PALETTE[i%PALETTE.length],
                            flexShrink:0,
                          }}>
                            {p.sym.slice(0,2)}
                          </div>
                      ...
                      <td style={{ padding:'12px 0', verticalAlign:'middle' }}>
                        <span style={{
                          ...
                          background: p.side==='long' ? 'rgba(52,211,153,0.1)' : 'rgba(244,63,94,0.1)',
                          color: p.side==='long' ? T.green : T.red,
                          border: `1px solid ${p.side==='long' ? 'rgba(52,211,153,0.2)' : 'rgba(244,63,94,0.2)'}`,
                        }}>
                          {p.side}
                        </span>
                      </td>
                      {/* Amount */}
                      <td style={{ ...mono, fontSize:12, color:T.slate, padding:'12px 0', verticalAlign:'middle' }}>{p.amount}</td>
                      {/* Avg Entry */}
                      <td style={{ ...mono, fontSize:12, color:T.slate, padding:'12px 0', verticalAlign:'middle' }}>{p.avgEntry}</td>
                      {/* Price */}
                      <td style={{ ...mono, fontSize:12, color:'var(--text)', padding:'12px 0', fontWeight:600, verticalAlign:'middle' }}>{p.price}</td>
                      {/* 24h */}
                      <td style={{ ...mono, fontSize:12, color:p.up ? T.green : T.red, padding:'12px 0', verticalAlign:'middle' }}>
                        <span style={{ display:'flex', alignItems:'center', gap:3 }}>
                          <span style={{ fontSize:9 }}>{p.up ? '▲' : '▼'}</span>
                          {p.ch}
                        </span>
                      </td>
                      {/* P&L */}
                      <td style={{ ...mono, fontSize:12, color:colorPnl(p.pnl), padding:'12px 0', fontWeight:600, verticalAlign:'middle' }}>{p.pnl}</td>
                      {/* Return */}
                      <td style={{ ...mono, fontSize:12, color:colorPnl(p.ret), padding:'12px 0', verticalAlign:'middle' }}>{p.ret}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
            ...
            <div style={{ display:'flex', flexDirection:'column', gap:12 }}>
              {sectorData.sort((a,b) => b.pct - a.pct).map((s, i) => (
                <div key={i} style={{ display:'grid', gridTemplateColumns:'110px 1fr 52px', alignItems:'center', gap:16 }}>
                  ...
                  <div style={{ ...mono, fontSize:11, color:T.slate, textAlign:'right' }}>
                    {s.pct.toFixed(1)}%
                  </div>
                </div>
              ))}
      ...
      {holdings.length > 0 && (
        <div style={{ display:'grid', gridTemplateColumns:`repeat(${Math.min(holdings.length,5)},1fr)`, gap:10 }}>
          {holdings.slice(0,5).map((h, i) => (
            <div key={i} className="aq-holding aq-card" style={{
              ...
              <div style={{
                ...
                background: `linear-gradient(90deg, ${PALETTE[i%PALETTE.length]}, transparent)`,
              }}/>
              <div style={{ display:'flex', justifyContent:'space-between', alignItems:'flex-start', marginBottom:10 }}>
                ...
                <span style={{
                  ...
                  color: h.up ? T.green : T.red,
                  padding:'2px 6px', borderRadius:4,
                  background: h.up ? 'rgba(52,211,153,0.08)' : 'rgba(244,63,94,0.08)',
                }}>
                  {h.ch}
                </span>
              </div>
              <div style={{ ...mono, fontSize:18, fontWeight:700, color:'var(--text)', marginBottom:10 }}>
                {h.price}
              </div>
